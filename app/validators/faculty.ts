...
const facultyMessageProvider = new SimpleMessagesProvider({
  ...
  'tin.regex': 'TIN should be numeric',
...
export const createFacultyValidator = vine.compile(
  vine.object({
    ...
    middleName: vine.string().maxLength(100).trim().optional(),
    surname: vine.string().maxLength(100).trim(),
    dateOfBirth: vine.date(),
    gender: vine.enum(['male', 'female']),
    image: vine.string().optional(),
    tin: vine.string().maxLength(100).regex(new RegExp('^[0-9]*$')).optional(),
    positionId: vine.number().min(1),
    employmentStatus: vine.enum([
      'regular',
      'part-time',
      'resigned',
      'terminated',
