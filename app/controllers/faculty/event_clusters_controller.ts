  ...
  async index({ view, request, response, auth }: HttpContext) {
    ...
    if (contentType === 'application/json') {
      try {
        ...
        const user = await this.facultyRepo.findById(auth.user?.id ?? 0)

        let events: EventCluster[] = []
        if (user) {
          events = await this.eventClusterRepo.getWithinRangeAndDepartment(
            query.from,
            query.to,
            user.departmentId
          )
    ...
    return view.render('faculty/event-clustering/index', {
      activeFaculty,
      departments,
    })
  }
